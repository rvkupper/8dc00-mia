...
def ls_solve(A, b):
    ...
    # compute the error
    A = np.transpose(util.c2h(np.transpose(A)))
    print(A,b)
...
def correlation(I, J):
    ...
    CC = (np.transpose(u).dot(v)) / (
        (np.transpose(u).dot(u))**0.5 * (
            np.transpose(v).dot(v))**0.5)
    return CC[0][0]
...
def mutual_information(I, Im_t, num_bins=16, minmax_range=None):
    # Compute the joint histogram of two signals.
    # Input:
    # I, Im_t - input images
    # num_bins: number of bins of the joint histogram (default: 16)
    ...
    # of the inputs)
    # Output:
    # MI - value of mutual information
    if I.shape != Im_t.shape:
        raise AssertionError("The inputs must be the same size.")

    # make sure the inputs are column-vectors of type double (highest
    # precision)
    I = I.reshape((I.shape[0]*I.shape[1],1)).astype(float)
    Im_t = Im_t.reshape((Im_t.shape[0]*Im_t.shape[1],1)).astype(float)

    # if the range is not specified use the min and max values of the
    # inputs
    if minmax_range is None:
        minmax_range = np.array([min(min(I),min(Im_t)), max(max(I),max(Im_t))])

    # this will normalize the inputs to the [0 1] range
    I = (I-minmax_range[0]) / (minmax_range[1]-minmax_range[0])
    Im_t = (Im_t-minmax_range[0]) / (minmax_range[1]-minmax_range[0])

    # and this will make them integers in the [0 (num_bins-1)] range
    I = np.round(I*(num_bins-1)).astype(int)
    Im_t = np.round(Im_t*(num_bins-1)).astype(int)

    """
        Part of original code of practicals, but not used in the mini project
    ...
    p = p/n
    #------------------------------------------------------------------#"""
    MI = metrics.mutual_info_score(I[:,0], Im_t[:,0])
    return MI
...
def mutual_information_e(p):
    ...
    #------------------------------------------------------------------#
    ent_I = -1 * np.sum(p_I * np.log(np.abs(p_I)))
    ent_J = -1 * np.sum(p_J * np.log(np.abs(p_J)))
    ent_IJ = -1 * np.sum(p*np.log(np.abs(p)))
